...
There's no GitHub remote for this repository yet, so these aren't filed
as actual issues — once one exists, file each of these using the
matching template in `.github/ISSUE_TEMPLATE/` (noted below) rather than
copy-pasting this file verbatim, so they're properly labeled and
trackable.
...
3. **Improve a formula parser error message.** Look at
   `packages/core/src/formula.ts`'s `CHEM002`/`CHEM003` error text and
   its tests in `packages/core/tests/formula.test.ts` — is there a
   confusing case (e.g. a lowercase-leading formula, a mismatched
   parenthesis) whose message could be clearer? Add a test for it too.

4. **Write a worked "Adding a Reaction" walkthrough** in `docs/` — a
...
5. **Add accessibility descriptions review** — read every entry in
...
6. **Add a CHEM008 documentation example** to `docs/error-codes.md` — the
...
7. **Create a schema example for an acid-base reaction with more than
...
8. **Add a lesson-idea seed** using the "Propose a lesson" issue
...
9. **Audit `datasets/elements/elements.yaml` against IUPAC's official
...
10. **Add a unit test for a formula the parser doesn't currently cover**
...
11. **Improve `apps/web`'s empty/loading states** — e.g. what does the
    "Visualize (conceptual)" section show while the equation input is
    empty? Small, contained UI polish in `apps/web/src/App.tsx`.

12. **Document `pnpm validate:data`'s output format** with a couple more
