...
| CHEM006 | ChemSpec schema validation failed — the YAML document doesn't match `reactionSchema` / `elementSchema` (wrong type, missing required field, bad enum value, duplicate id, etc).             | `validate:data`                                |
